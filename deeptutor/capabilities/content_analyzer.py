...
ENGLISH_ANALYZE_PROMPT = """你是一位资深英语教师和中考命题专家。请对以下英语教材内容进行深度拆解分析。

请以 JSON 格式返回以下结构（不要其他文字）：
{{
  "topic": "主题",
  "vocabulary": [
    {{"word": "单词", "phonetic": "音标", "definition": "中文释义", "example": "例句"}}
  ],
  "grammar": [
    {{"point": "语法点", "explanation": "解释", "examples": ["例句1"], "common_mistakes": "常见错误"}}
  ],
  "reading_skills": ["阅读技巧"],
  "exam_points": [
    {{"point": "考点", "type": "词汇|语法|阅读|听力|写作", "difficulty": "easy|medium|hard"}}
  ]
}}
...
SCIENCE_ANALYZE_PROMPT = """你是一位资深科学教师（物理/化学/生物）和中考命题专家。请对以下科学教材内容进行深度拆解分析。
...
{{
  ...
  "chapter": "所属章节",
  "concepts": [
    {{"name": "概念名", "definition": "定义", "key_points": ["要点"]}}
  ],
  "formulas": [
    {{"formula": "公式", "description": "含义", "conditions": "适用条件"}}
  ],
  "experiments": [
    {{"name": "实验名", "purpose": "目的", "steps": ["步骤"], "conclusion": "结论"}}
  ],
  "exam_points": [
    {{"point": "考点", "type": "概念理解|计算|实验分析|应用", "difficulty": "easy|medium|hard"}}
  ]
}}
...
SOCIAL_ANALYZE_PROMPT = """你是一位资深社科教师（历史/地理/政治）和中考命题专家。请对以下社科教材内容进行深度拆解分析。
...
{{
  ...
  "chapter": "所属章节",
  "timeline": [
    {{"event": "事件", "date": "时间", "significance": "意义"}}
  ],
  "concepts": [
    {{"name": "概念名", "definition": "定义", "significance": "重要性"}}
  ],
  "causes_effects": [
    {{"cause": "原因", "effect": "影响"}}
  ],
  "exam_points": [
    {{"point": "考点", "type": "识记|理解|分析|综合", "difficulty": "easy|medium|hard"}}
  ]
}}
...
class ContentAnalyzerCapability(BaseCapability):
    ...
    def _extract_graph_nodes(
        ...
        if content_type == ContentType.LITERARY:
            ...
            for vocab in analysis.get("vocabulary", []):
                nodes.append({"id": vocab.get("word", ""), "type": "vocabulary"})
            for grammar in analysis.get("grammar", []):
                nodes.append({"id": grammar.get("point", ""), "type": "grammar"})
            ...
            for exp in analysis.get("experiments", []):
                nodes.append({"id": exp.get("name", ""), "type": "experiment"})
        elif content_type == ContentType.SOCIAL:
            for concept in analysis.get("concepts", []):
                nodes.append({"id": concept.get("name", ""), "type": "concept"})
            for event in analysis.get("timeline", []):
                nodes.append({"id": event.get("event", ""), "type": "event"})
        return nodes
    ...
    def _get_analyze_prompt(content_type: str) -> str:
        ...
        prompts = {
            ...
            ContentType.ENGLISH: ENGLISH_ANALYZE_PROMPT,
            ContentType.SCIENCE: SCIENCE_ANALYZE_PROMPT,
            ContentType.SOCIAL: SOCIAL_ANALYZE_PROMPT,
        }
    ...
    def _format_output(structured: dict[str, Any], content_type: str) -> str:
        ...
        if content_type == ContentType.LITERARY:
            ...
            if content_type == ContentType.ENGLISH:
                for v in analysis.get("vocabulary", []):
                    lines.append(f"- **{v.get('word', '')}** [{v.get('phonetic', '')}]: {v.get('definition', '')}")
                for g in analysis.get("grammar", []):
                    lines.append(f"- 📐 **{g.get('point', '')}**: {g.get('explanation', '')}")
            elif content_type == ContentType.SCIENCE:
                for c in analysis.get("concepts", []):
                    lines.append(f"- **{c.get('name', '')}**: {c.get('definition', '')}")
                for f in analysis.get("formulas", []):
                    lines.append(f"- `{f.get('formula', '')}` — {f.get('description', '')}")
                for e in analysis.get("experiments", []):
                    lines.append(f"- 🔬 **{e.get('name', '')}**: {e.get('purpose', '')}")
            elif content_type == ContentType.SOCIAL:
                for event in analysis.get("timeline", []):
                    lines.append(f"- **{event.get('date', '')}** {event.get('event', '')}: {event.get('significance', '')}")
                for c in analysis.get("concepts", []):
                    lines.append(f"- **{c.get('name', '')}**: {c.get('definition', '')}")
            else:
