...
from deeptutor.api.routers.auth import get_current_user
...
def _session_scope(user: dict) -> tuple[str, bool]:
    username = str(user.get("username") or user.get("sub") or "pigouwu")
    return username, bool(user.get("is_admin"))
...
async def list_sessions(
    ...
    user_id, include_all = _session_scope(user)
    sessions = await store.list_sessions(limit=limit, offset=offset, user_id=user_id, include_all=include_all)
...
async def get_session(session_id: str, user: dict = Depends(get_current_user)):
    store = get_sqlite_session_store()
    user_id, include_all = _session_scope(user)
    session = await store.get_session_with_messages(session_id, user_id=user_id, include_all=include_all)
    if session is None:
...
async def rename_session(session_id: str, payload: SessionRenameRequest, user: dict = Depends(get_current_user)):
    store = get_sqlite_session_store()
    user_id, include_all = _session_scope(user)
    updated = await store.update_session_title(session_id, payload.title, user_id=user_id, include_all=include_all)
    if not updated:
        raise HTTPException(status_code=404, detail="Session not found")
    session = await store.get_session(session_id, user_id=user_id, include_all=include_all)
...
async def delete_session(session_id: str, user: dict = Depends(get_current_user)):
    store = get_sqlite_session_store()
    user_id, include_all = _session_scope(user)
    deleted = await store.delete_session(session_id, user_id=user_id, include_all=include_all)
    if not deleted:
        raise HTTPException(status_code=404, detail="Session not found")
